...
import numpy as np
import meshio
from models.losses.losses import relativemmd,mmd
...
for wrapper, name in d.items():
    ...
    np.save("geometrical_measures/latent_"+name+".npy",latent_space.numpy())
    ...
    mmd_data=mmd(temp.reshape(NUMBER_SAMPLES,-1),data2.reshape(NUMBER_SAMPLES,-1))
    np.save("geometrical_measures/moment_tensor_data.npy",moment_tensor_data)
    np.save("geometrical_measures/moment_tensor+"+name+".npy",moment_tensor_sampled)
    np.save("geometrical_measures/area_data.npy",area_data)
    np.save("geometrical_measures/area_+"+name+".npy",area_sampled)
    np.save("geometrical_measures/variance_+"+name+".npy",variance)
    np.save("geometrical_measures/variance_data.npy",variance_data)
    np.save("geometrical_measures/mmd_+"+name+".npy",mmd_data)
    np.save("geometrical_measures/rel_error+"+name+".npy",error)
